...
When we walk into quantum mechanics, we should consider von-Neumann entropy $S = -Tr(\rho ln\rho)$. Let we show von-Neumann entropy is an extension of Shannon entropy(Gibbs entropy)
$$
S = -Tr(\rho ln\rho)\\
if \rho = \sum_i p_i|i\rangle\langle i|\\
S = -Tr[(\sum_i p_i|i\rangle\langle i|)ln(\sum_i p_i|i\rangle\langle i|)]\\
S = -Tr[(\sum_i p_i|i\rangle\langle i|)(\sum_i lnp_i|i\rangle\langle i|)]\\
S = -Tr(\sum_i p_ilnp_i|i\rangle\langle i|)=-\sum_i p_ilnp_i
$$
Now let we see what we can get from *Maximum Entropy Principle* with von-Neumann entropy. The answer is Bose-Einstein distribution and Fermi-Dirac distribution.
