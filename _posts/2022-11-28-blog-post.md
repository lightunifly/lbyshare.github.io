...
$$
\rho = \frac{e^{-\beta(\hat{H}-\mu\hat{N})}}{{Tr}(e^{-\beta(\hat{H}-\mu\hat{N})})}
$$
...
so
$$
p_i = \rho_{ii} = \frac{e^{-\beta(H_i-\mu N_i)}}{\sum_i(e^{-\beta(H_i-\mu N_i)})}
$$
but this is Maxwell-Boltzmann's distribution?
